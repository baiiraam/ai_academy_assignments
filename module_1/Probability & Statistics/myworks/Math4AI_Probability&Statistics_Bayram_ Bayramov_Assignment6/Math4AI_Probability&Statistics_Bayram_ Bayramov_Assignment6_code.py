...
def plot_variance_bias(
    ...
    bins = 40
    plt.hist(sigma2_mle, bins=bins, density=True, alpha=0.6, label=r"$\hat{\sigma}^2_{MLE}$", color='blue')
    plt.hist(s2_unbiased, bins=bins, density=True, alpha=0.6, label=r"$S^2$ (unbiased)", color='orange')
    plt.axvline(1.0, linestyle="--", linewidth=2, color='red', label="True variance = 1")
...
def plot_gumbel_fit(data: np.ndarray, mu_hat: float, beta_hat: float, filename: str) -> None:
    ...
    plt.plot(xs, pdf, linewidth=2, color='red',
...
def plot_bootstrap_hist(
    ...
    plt.figure(figsize=(9, 5))
    plt.hist(boot_stats, bins=40, density=True, alpha=0.7, color='skyblue', label="Bootstrap statistics")
    plt.axvline(ci_low, linestyle="--", linewidth=2, color='red', label=f"CI low = {ci_low:.3f}")
    plt.axvline(ci_high, linestyle="--", linewidth=2, color='red', label=f"CI high = {ci_high:.3f}")
    plt.axvline(np.median(boot_stats), linestyle=":", linewidth=1.5, color='black', label=f"Mean bootstrap median = {np.mean(boot_stats):.3f}")
...
def bonus_bootstrap_coverage_study(
    ...
    for N in sample_sizes:
        ...
        print(f"\nSample size N = {N}:")
        print(f"  Coverage probability: {coverage_prob:.4f} ± {results[N]['coverage_se']:.4f}")
...
def main() -> None:
    ...
    print(f"  difference        = ({mu_hat - mu_scipy:.6f}, {beta_hat - beta_scipy:.6f})")
